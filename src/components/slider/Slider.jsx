        ...
        {sliderChallenges.map((challenge) => {
          return (
            <SwiperSlide key={challenge._id}>
              <div
                className={`w-full h-full bg-cover bg-center bg-no-repeat flex flex-col gap-20 justify-center items-center font-bold text-3xl md:text-4xl lg:text-5xl relative`}
                style={{ backgroundImage: `url(${challenge.imageUrl})` }}
              >
                <div className="w-full h-full bg-black absolute opacity-50 z-0"></div>
                <div className="z-10 text-white">{challenge.description}</div>
                <button
                  onClick={() => handleViewChallenge(challenge._id)}
                  className="btn bg-secondary hover:bg-base-300 z-10 text-black"
                >
                  View Challenge
                </button>
              </div>
